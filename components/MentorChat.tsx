...
const MessageRenderer: React.FC<{ content: string; sources?: any[] }> = ({ content, sources }) => {
  const isError = content.includes('[LIMIT KUOTA TERLAMPAU]') || content.includes('[ERROR]');
  
  const parts = content.split(/(```[\s\S]*?```)/g);
  return (
    <div className={`space-y-6 ${isError ? 'bg-rose-500/5 p-6 rounded-3xl border border-rose-500/20' : ''}`}>
      {parts.map((part, index) => {
        if (part.startsWith('```')) {
          const match = part.match(/```(\w+)?\n?([\s\S]*?)```/);
          return <CodeBlock key={index} code={match?.[2] || ''} language={match?.[1] || 'code'} />;
        }
        return <p key={index} className={`whitespace-pre-wrap leading-relaxed ${isError ? 'text-rose-200' : 'text-slate-200'} text-lg font-medium`}>{part}</p>;
      })}
